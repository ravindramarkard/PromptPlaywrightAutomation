...
import { FiX, FiSave, FiEdit3, FiClipboard, FiBarChart2 } from 'react-icons/fi';
...
const EditTestSuiteModal = ({ 
  ...
  useEffect(() => {
    if (isOpen && testSuite) {
      setFormData({
        ...
        testType: testSuite.testType || 'UI'
      });
      
      // Pre-populate selected tests from existing test suite
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updatedData = {
        ...formData,
        testCases: selectedTests, // Update testCases array
        selectedTestCases: selectedTests,
        totalTests: selectedTests.length,
        status: testSuite.status || 'ready'
      };
